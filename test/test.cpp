...
#define DLRTEST_IS_LOCKED() (lockedX >= 0 && lockedY >= 0)
    ...
    for (int i = 0; i < num_envs; ++i) {
        ...
        Uint32 c = ( ! (bg && curX >= 0)) ? 0 : DLR_ReadPixel32_Raw(bg->pixels, bg->pitch, 4, curX, curY);
        Uint8 a, r, g, b;
            ...
            if (dest) {
                for (int y = 0; y < dest->h; ++y) {
                    for (int x = 0; x < dest->w; ++x) {
                        ...
                        if (A && B) {
                            Uint32 ac = DLR_ReadPixel32_Raw(A->pixels, A->pitch, 4, x, y);
                            int aa, ar, ag, ab;
                            DLR_SplitARGB32(ac, aa, ar, ag, ab);

                            Uint32 bc = DLR_ReadPixel32_Raw(B->pixels, B->pitch, 4, x, y);
                            int ba, br, bg, bb;
                        ...
                        DLR_WritePixel32_Raw(dest->pixels, dest->pitch, 4, x, y, dc);
                    }
                }
            }
        }
